...
Failure screenshots, traces, videos, and the HTML report are generated under `e2e/test-results/` and `e2e/playwright-report/`; both are ignored by Git. Lab 2 evidence remains under `artifacts/lab-02/`; the authenticated requester evidence generated by #50 belongs under `artifacts/lab-03/screenshots/` and is committed separately from temporary Playwright diagnostics.
