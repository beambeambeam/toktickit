...
Final evidence was regenerated on 2026-09-03 with `pnpm test:e2e`: Requester Selection (6 PNGs), Create Ticket (21 PNGs), My Tickets (24 PNGs), and Ticket Detail (21 PNGs). Each viewport manifest lists 24 unique captures. The artifacts cover desktop, tablet, mobile, initial/selected, validation, loading, API failure, busy submission, invalid Attachment, empty/no-results, ownership isolation, direct unauthorized Ticket refusal, active/uploading/uploaded/removed Attachment, and removal confirmation states. The E2E flow also verifies that removed Attachment content returns 404. The visual checklist records the final screenshot review and distinguishes machine-asserted rows from human-judged rows.
...
- No screenshot is reused under multiple state names. The E2E manifest is the complete source list; the report selects distinct representative captures so each visual state appears once in the PDF.
