...
| API-04 | API/integration | server/tests/lab-03/ticket-comments.api.test.ts; server/tests/lab-03/comments-notes.api.test.ts | Role visibility, backend attribution, empty/1/5000/5001 code points, HTML payload stored as text, stable ordering, bounded reads, terminal write race, no edit/delete/status side effect or private metadata leak. | Partial — public comments API coverage implemented; private notes and broader role/concurrency matrix remain planned |
...
| UI-05 | UI component | client/tests/lab-03/staff-ticket-detail.test.tsx | Read-only operational detail, active/removed Attachment states, download, invalid-ID and retry failures; workflow, ownership/IT Priority controls, public comments and later operational composers. | Partial — #52 ownership/IT Priority, #53 status workflow and #54 public comment interactions pass; private notes and later operational modes remain planned |
