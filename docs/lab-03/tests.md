...
| API-03 | API/integration | server/tests/lab-03/ticket-ownership.api.test.ts; server/tests/lab-03/staff-ticket-detail.api.test.ts | Claim one winner, reassignment/unassignment, eligible admin owner, stale/terminal denial, independent priority, every status edge, timestamps, idempotent indication/reopen and account-change races. | Partial — #52 ownership and IT Priority API coverage passes; status, indication and account-change cases remain planned |
...
| UI-05 | UI component | client/tests/lab-03/staff-ticket-detail.test.tsx | Read-only operational detail, active/removed Attachment states, download, invalid-ID and retry failures; ownership/IT Priority controls and later operational composers. | Partial — #52 ownership/IT Priority interactions pass; status, comments, notes and indication remain planned |
...
| E2E-03 | E2E | e2e/lab-03/staff-queue.spec.ts; e2e/lab-03/ticket-ownership.spec.ts; e2e/lab-03/staff-ticket-flow.spec.ts | Queue/detail, claim/priority, status/reopen/indication, public Requester-staff conversation, private staff/admin-only notes and terminal denial. | Partial — #51 queue-to-detail and #52 claim/priority journeys are implemented; later operational actions remain planned |
