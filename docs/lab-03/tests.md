...
| API-04 | API/integration | server/tests/lab-03/internal-notes.api.test.ts | Private-note role visibility, backend attribution, empty/1/5000/5001 code points, HTML payload stored as text, stable ordering, terminal write race, no edit/delete/status side effect or public metadata leak. | Partial — private notes API coverage implemented; public comments and broader role/concurrency matrix remain planned |
...
| UI-05 | UI component | client/tests/lab-03/staff-ticket-detail.test.tsx; client/tests/lab-03/internal-notes.test.tsx | Read-only operational detail, active/removed Attachment states, download, invalid-ID and retry failures; private-note composer states and later operational modes. | Partial — detail and private-note states pass; public comments/status/indication modes remain planned |
...
| E2E-03 | E2E | e2e/lab-03/staff-ticket-flow.spec.ts; e2e/lab-03/staff-queue.spec.ts; e2e/lab-03/internal-notes.spec.ts | Queue/detail, claim/priority, status/reopen/indication, public Requester-staff conversation, private staff/admin-only notes and terminal denial. | Partial — #51 queue-to-detail and #55 private-note journeys are covered; remaining operational slices are planned |
